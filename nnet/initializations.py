...
def scaled_uniform_(tensor):
    return init.kaiming_uniform_(tensor, a=math.sqrt(5))

# N(0, std**2) where std = sqrt(1/fan_in)
def scaled_normal_(tensor):
    return init.kaiming_uniform_(tensor, nonlinearity="linear")
...
def lecun_normal_(tensor):
    return init.kaiming_normal_(tensor, nonlinearity="linear")

# U(-b, b) where b = sqrt(6/fan_in)
def he_uniform_(tensor):
    return init.kaiming_uniform_(tensor)

# N(0, std**2) where std = sqrt(2/fan_in)
def he_normal_(tensor):
    return init.kaiming_normal_(tensor)
