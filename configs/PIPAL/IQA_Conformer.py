...
class CNN_Model(nnet.Regressor):
    ...
    def __init__(self):
        ...
        # Emb
        self.emb_p = nnet.Embedding(num_embeddings=H * W, embedding_dim=dim_model)

        # Encoder
        self.encoder = nnet.Conformer(
            dim_model=[dim_model],
            num_blocks=[1],
            ff_ratio=ff_ratio,
            att_params={"class":"MultiDimensionalMultiHeadAttention", "num_heads": 4},
            conv_params={"class": "Conv2d", "params": {"padding": "same","kernel_size": (7, 7)}}
        )

        # Decoder
        self.decoder = nnet.CrossConformer(
            dim_model=[dim_model],
            num_blocks=[1],
            ff_ratio=ff_ratio,
            att_params={"class":"MultiDimensionalMultiHeadAttention", "num_heads": 4},
            cross_att_params={"class":"MultiDimensionalMultiHeadAttention", "num_heads": 4},
            conv_params={"class": "Conv2d", "params": {"padding": "same","kernel_size": (7, 7)}}
        )

        # Head
        self.head = nn.Sequential(
            nnet.GlobalAvgPool2d(dim=(1, 2)),
            nnet.Linear(in_features=dim_model, out_features=dim_model),
            nnet.ReLU(),
            nnet.Linear(in_features=dim_model, out_features=1),
        )

        self.trainable = [self.conv_proj, self.emb_p, self.encoder, self.decoder, self.head]
    ...
    def forward(self, x):
        ...
        x_ref = x_ref.permute(0, 2, 3, 1)
        x_diff = x_diff.permute(0, 2, 3, 1)

        x_ref += self.emb_p(torch.arange(torch.prod(torch.tensor(x_ref.shape[1:-1])), device=x_ref.device).reshape(x_ref.shape[1:-1]))
        x_diff += self.emb_p(torch.arange(torch.prod(torch.tensor(x_diff.shape[1:-1])), device=x_diff.device).reshape(x_diff.shape[1:-1]))
    ...
    def generate(self, dataset, saving_path=None):
        ...
        with open(os.path.join(saving_path, "readme.txt"), "w") as f:
            ...
            description += "Other description : Solution based on an inception resnet v2 network pretrained on ImageNet and a Convolutional Transformer post Network. Distorted images MOS are regressed using a Mean Squared Error loss. The method was trained on the PIPAL dataset training subset.\n"
...
callback_path = "callbacks/PIPAL/IQA_Conformer/"
generation_path = "callbacks/PIPAL/IQA_Conformer/eval/"
